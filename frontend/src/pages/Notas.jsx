...
const Notas = () => {
    ...
    useEffect(() => {
        const storedAsignaturas = JSON.parse(localStorage.getItem(`asignaturas_${userId}`));
    ...
    const guardarAsignaturasEnLocalStorage = (asignaturasData) => {
        localStorage.setItem(`asignaturas_${userId}`, JSON.stringify(asignaturasData));
    };

    const agregarAsignatura = () => {
        const newAsignaturas = [...asignaturas, { nombre: "", notas: [], porcentajes: [] }];
    ...
    const handlePorcentajesChange = (e, asignaturaIndex, porcentajeIndex) => {
        const newAsignaturas = [...asignaturas];
        newAsignaturas[asignaturaIndex].porcentajes[porcentajeIndex] = Number(e.target.value);
        setAsignaturas(newAsignaturas);
    ...
    const calcularPromedioAsignatura = (notas, porcentajes) => {
        const totalNotas = notas.reduce((acc, nota, index) => acc + (nota * porcentajes[index]), 0);
        const totalPorcentajes = porcentajes.reduce((acc, porcentaje) => acc + porcentaje, 0);
        ...
        <>
            ...
            <div className="z-10 mt-[80px] w-[100vw] max-w-[100vw] text-current lg:w-[55vw] lg:max-w-[90vw] lg:px-10  ">
                <div className="mt-4 flex-col items-center justify-between m-4 rounded-lg bg-foreground text-[5vw] font-bold sm:text-[18px] ">
                    ...
                    <div className="flex items-center justify-center py-2">
                        <div className="text-sm items-center justify-center">
                            ...
                            <input
                                ...
                                maxLength="2"
                                value="65"
                                className="w-14 px-2 py-1 rounded-md border border-gray-300 "
                                readOnly
                            />
                            <span className=""> </span>
                            <input
                    ...
                    <div className="bg-foreground rounded-[20px] ">
                        <div className="flex flex-col items-center justify-center p-4">
                                ...
                                <div key={asignaturaIndex} className="m-2">
                                    <div className="flex items-center justify-center  p-2">
                                        <input
                                            ...
                                            onChange={(e) => handleAsignaturaChange(e, asignaturaIndex)}
                                            placeholder="Nombre de la Asignatura"
                                            className="w-4/4  py-1 rounded-md border border-gray-300"
                                            maxLength="32"
                                        />
                                    </div>
                                    {asignatura.notas.map((nota, notaIndex) => (
                                        <div key={notaIndex} className="flex space-x-2 mb-2 items-center justify-center">
                                            ...
                                            <input
                                                ...
                                                value={nota || ''}
                                                onChange={(e) => handleNotasChange(e, asignaturaIndex, notaIndex)}
                                                placeholder="Nota"
                                            />
                                            <input
                                                pattern="[0-9]*"
                                                ...
                                                onChange={(e) => handlePorcentajesChange(e, asignaturaIndex, notaIndex)}
                                                placeholder="%"
                                            />
                                            <span className="ml-2">%</span>
                                            <button className="ml-2 text-red-500 font-bold" onClick={() => eliminarNota(asignaturaIndex, notaIndex)}>X</button>

                                        </div>
                                    ))}
                                    <div className="flex flex-col justify-center items-center">
                                        {asignatura.notas.length > 0 && (
                                            <p className="" >Promedio: {calcularPromedioAsignatura(asignatura.notas, asignatura.porcentajes).toFixed(1)}</p>
                                        )}
                                        <div className="flex gap-4">
                                            <AiFillPlusCircle
                                                onClick={() => agregarNota(asignaturaIndex)}></AiFillPlusCircle>
                                            <AiFillDelete
                            ...
                            <button
                                className="flex m-2  hover:bg-background rounded-lg p-4"
